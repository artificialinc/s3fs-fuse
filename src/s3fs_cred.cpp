...
bool S3fsCred::CheckIAMCredentialUpdate(std::string* access_key_id, std::string* secret_access_key, std::string* access_token)
{
    ...
    if(access_token){
        if(IsIBMIAMAuth() || IsSetIAMRole(AutoLock::ALREADY_LOCKED) || is_ecs || is_use_session_token || IsSetExtCredLib()){
...
bool S3fsCred::CheckAllParams()
{
    ...
    if(!S3fsCurl::IsPublicBucket() && !load_iamrole && !is_ecs && !S3fsCred::IsSetExtCredLib()){
